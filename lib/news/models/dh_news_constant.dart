import 'package:flutter/material.dart';

Color tabBarUnSelectedTextColor = Color(0xff999999);
Color tabBarSelectedTextColor = Color(0xffC8003D);
Color textColor = Color(0xff333333);
Color seperationColor = Color(0xffE0E0E0);
Color sourceTextColor = Color(0xff828282);


// DH API creds
final String partnerCode = 'snapdeal1';
final String apiKeyValue = 'grBY9UjGjFgpXpFyOnuwNPMPXmxp+y5/HjxMrgFixu8=';
final String secretKey = 'xurDYa6wyY+zw3NzBh/YfQ==';
final String baseUrl = 'https://feed.dailyhunt.in/api/v2/syndication';

final String headlineID = '1';
final String entertainmentID = '4178';
final String dailyShareID = '4324';
final bool   runModuleIndependent = true;



